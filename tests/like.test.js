...
describe('post like handler', function () {
  var userId;
  var articleId;
  var token;
  ...
  it('responds with 201 when submitting correct info', function (done) {
    request(app).post(`/api/likes/`)
      ...
      .end(function (err, res) {
        ...
        db.query('SELECT * FROM `like` WHERE `user_id`=? AND `article_id`=?', [userId, articleId], function (error, results) {
  ...
  it('responds with 422 when like already exists', function (done) {
    request(app).post(`/api/likes/`)
      .set({
        Authorization: `Bearer ${token}`
      })
  ...
  it('responds with 422 when article id is invalid, and body contains error msg(s)', function (done) {
    request(app).post(`/api/likes/`)
      .set({
        Authorization: `Bearer ${token}`
      })
      .send({
        article_id: "incorrect351325"
      })
      .expect(422)
      .expect('Content-Type', /json/)
      .end(function (err, res) {
